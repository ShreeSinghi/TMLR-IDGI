    ...
    for i in tqdm(range(opt.resume_epoch, 1000, CLASS_CLUSTER)): # We are processing 20 classes at a time
        ...
        for class_idx in range(i, i+CLASS_CLUSTER):
            ...
            for j, n_step in enumerate(compute_at):
                np.save(os.path.join(opt.dataroot, "saliencies", opt.model,   f"{opt.method}_ig", f"class_{class_idx:03d}_steps_{n_step}"), salienciency_ig[j][pointer:pointer+class_size])
                np.save(os.path.join(opt.dataroot, "saliencies", opt.model,   f"{opt.method}_idgi", f"class_{class_idx:03d}_steps_{n_step}"), salienciency_idgi[j][pointer:pointer+class_size])
            pointer += class_size
